...
#: La fecha que la siembra considera "hoy". La usan los diez lugares que
#: siembran datos del día — reservas, movimientos de caja, ventas.
#:
#: 🔴 **Se refresca en `sembrar()`, NO al importar el módulo.** Era
#: `HOY = date.today()` a secas acá arriba, o sea congelada en el instante del
#: import. La suite tarda seis minutos: importar antes de medianoche y sembrar
#: después significa **sembrar para ayer y después preguntar por hoy**. Fue
#: exactamente lo que puso en rojo el CI de la promoción a producción del
#: 2026-08-29, a las 00:04 de Argentina, con el mismo código que había pasado
#: en verde una hora antes. Se lee como un test inestable y es un dato viejo.
#:
#: Y no es sólo el test: la demo se resiembra por cron sobre un proceso que
#: puede vivir días. Con la fecha del import, «una reserva para hoy» termina
#: siendo una reserva para el día que arrancó el proceso.
HOY = date.today()
...
def sembrar(api: Api) -> date:
    """Siembra la demo y **devuelve la fecha que usó como «hoy»**.

    Devolverla no es decorativo: quien verifique que existe «una reserva para
    hoy» tiene que preguntar por **la misma fecha con la que se sembró**, no por
    la que sea `date.today()` cuando le toque mirar.
    """
    hoy = _fijar_hoy()
    hechos = {}
