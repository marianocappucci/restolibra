...
**Y acá hay una segunda línea que se puede olvidar**: este producto no pasa por
`json_api_require_role` de libraauth, tiene su propio guard en
`app/web/api_auth.py`. La llamada a `exigir_terminos` está escrita ahí a mano.
...
def test_una_llamada_gateada_corta_hasta_aceptar(admin_client):
    respuesta = admin_client.get("/api/clientes")
...
def test_aceptar_destraba_la_instancia(admin_client):
    estado = admin_client.get("/api/terminos").json()
    assert estado["pendiente"] is True
    assert estado["puede_aceptar"] is True

    aceptada = admin_client.post("/api/terminos/aceptar", json={"version": estado["version"]})
    assert aceptada.status_code == 200, aceptada.text
    assert aceptada.json()["pendiente"] is False

    assert admin_client.get("/api/clientes").status_code == 200
...
def test_el_camino_para_salir_del_gate_no_se_gatea_a_si_mismo(admin_client):
    ...
    assert admin_client.get("/api/terminos").status_code == 200
...
def test_la_fila_probatoria_guarda_version_hash_y_quien(admin_client):
    ...
    estado = admin_client.get("/api/terminos").json()
    admin_client.post("/api/terminos/aceptar", json={"version": estado["version"]})

    historial = admin_client.get("/api/terminos/historial").json()
