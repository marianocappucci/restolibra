"""Entorno de Alembic para las tablas **propias** de Restolibra.

De las 67 tablas de una instancia, esta cadena maneja **9**: `venta_links`,
`recetas`, `receta_items`, `salones`, `mesas`, `pedidos`, `comandas`,
`pedido_items` y `reservas`. Las otras 58 son de los motores y cada uno las
mantiene por su cuenta — ver `app/schema_propio.py`, que tiene el reparto
completo.
...
1. **`version_table` propia.** `libracore-migrar` corre contra **esta misma
   ...
   leyera la revisión de la otra: la del motor vería `0001_baseline_restolibra`,
   no la reconocería, y el deploy moriría. Va `alembic_version_restolibra`, que
...
3. **No hay modo offline.** `alembic upgrade --sql` no puede emitir nada útil:
   ...
   DDL (los tres `ALTER` guardados por introspección del final de
   `init_schema_propio`, y el runner de migraciones que corre
   `init_commerce_schema`). Un `--sql` que emitiera algo estaría mintiendo, así
   que corta. Es la misma decisión que tomó el `env.py` de LibraCore.

El destino sale de `url_de_instancia("restolibra")`, que es **el mismo lugar**
...
VERSION_TABLE = "alembic_version_restolibra"
...
def destino() -> str:
    ...
    """
    url = url_de_instancia("restolibra") or context.config.get_main_option(
        "sqlalchemy.url", default=""
    )
    if not url or url.startswith("postgresql://user:password@"):
        raise RuntimeError(
            "Falta la URL de la base de Restolibra: definí "
            "RESTOLIBRA_DATABASE_URL. Sin eso no hay base contra la cual migrar "
