"""Baseline: las tablas propias de Restolibra tal como ya existen.

Esta revisión **llama a `init_schema_propio()`** en vez de re-expresar sus 9
...
🔴 **Por qué llama a `init_commerce_schema()` primero.** Varias de las tablas
propias tienen FK contra tablas de LibraCommerce: `venta_links` y `pedidos`
contra `sales`, y `recetas`/`receta_items`/`pedido_items` contra
`catalog_items`. Y LibraCommerce **no tiene cadena de Alembic**: sus tablas
...
revision = "0001_baseline_restolibra"
down_revision = None
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    # arranca `init_schema()` de LibraCommerce revienta contra PostgreSQL, y los
    # `PRAGMA table_info(...)` de los ALTER de `init_schema_propio` también.
...
def downgrade():
    # Bajar de la baseline es borrar el salón entero: mesas, pedidos, comandas y
    # recetas. No hay caso de uso que lo justifique y sí una forma muy barata de
    # perder una instancia: el rollback de esta revisión es restaurar el backup.
